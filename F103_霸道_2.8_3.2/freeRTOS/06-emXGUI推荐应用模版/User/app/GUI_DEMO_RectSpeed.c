...
void GUI_DEMO_RectSpeed(void)
{
  ...
  NumPixels=0;
   ...
   hdc_mem = CreateMemoryDC(SURF_SCREEN, GUI_XSIZE, GUI_YSIZE);
   ClrDisplay(hdc_mem,NULL,MapRGB(hdc,0,100,200));
	...
	while(1)
	{
		...
		SetBrushColor(hdc_mem,_aColor[i&7]);
	...
	//	r.h = 60;//r.y + rand() % YSize;
		...
		FillRect(hdc_mem,&r);
    BitBlt(hdc, 0, 0, GUI_XSIZE, GUI_YSIZE, hdc_mem, 0, 0, SURF_SCREEN);
